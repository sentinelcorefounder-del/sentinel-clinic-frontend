...
function badgeClass(status: string) {
  ...
  if (["paid", "issued", "completed"].includes(s))
  ...
  if (["failed", "exception", "ops_rejected", "cancelled"].includes(s))
...
function Badge({ value }: { value: string }) {
  return (
    <span
      className={`inline-flex rounded-full px-2 py-1 text-xs font-semibold ${badgeClass(value)}`}
    >
      {(value || "-").replaceAll("_", " ")}
    </span>
  );
...
export default async function OpsPatientsPage({ searchParams }: Props) {
  ...
  const search = valueOf(sp.search);
  const hospital = valueOf(sp.hospital);
  const clinic = valueOf(sp.clinic);
  ...
  return (
    <div>
      <h1 className="mb-2 text-3xl font-bold">Ops Patient Registry</h1>
                ...
                {patients.map((p: any) => (
                  <tr key={p.id} className="border-t">
                    ...
                    <div className="text-xs text-slate-500">
                      Sentinel Master Patient ID: {p.sentinel_patient_id || "Identity pending"}
                    </div>
                    </td>
                    <td className="p-3">
                      <div>{p.source_hospital || "-"}</div>
                      <div className="text-xs text-slate-500">
                        {p.source_hospital_code || ""}
                      </div>
                    </td>
                    <td className="p-3">
                      <div>{p.assigned_clinic || "-"}</div>
                      <div className="text-xs text-slate-500">
                        {p.assigned_clinic_code || ""}
                      </div>
                    </td>
                    <td className="p-3">
                      <Badge value={p.referral_status || "-"} />
                    </td>
                    <td className="p-3">
                      <Badge value={p.payment_status || "not_created"} />
                    </td>
                    <td className="p-3">
                      <Badge value={p.report_status || "not_created"} />
                    </td>
                    <td className="p-3">{p.images_count || 0}</td>
                    <td className="p-3">
                      <Link
                        href={`/ops/patients/${p.id}`}
