...
export default function OnwardReferralManager({ encounterId, encounterReference, patientPhone, user }: {
  ...
  const [credentials, setCredentials] = useState({ clinician_name: "", professional_role: "Optometrist", registration_number: "", reason: "" });
  const [message, setMessage] = useState("");
  const [busy, setBusy] = useState(false);
  const exactOptometrist = Boolean(user && !user.is_superuser && user.roles.includes("optometrist"));
  const clinicAdmin = Boolean(user && !user.is_superuser && user.roles.includes("clinic_admin"));
  const canDistribute = exactOptometrist || Boolean(user && !user.is_superuser && user.roles.includes("clinic_admin") && user.roles.includes("onward_referral_distributor"));
  ...
  return (
    <section className="space-y-5 rounded-xl border bg-white p-6">
      ...
      <div className="grid gap-3 text-sm md:grid-cols-3"><p><b>Assessment completed:</b> {eligibility.encounter_completed ? "Yes" : "No"}</p><p><b>Eligible professional sources:</b> {sourceLabel}</p><p><b>Responsible optometrist:</b> {eligibility.responsibility?.clinician_name || "Not accepted"}</p></div>

      {!eligibility.responsibility && exactOptometrist ? <div className="space-y-3 rounded border border-amber-300 bg-amber-50 p-4"><p className="font-semibold">Explicit clinical responsibility required</p><div className="grid gap-3 md:grid-cols-2">
