...
export default function FinanceSetupWizard() {
  ...
  async function completeSetup() {
    ...
    try {
      ...
      const wallet = await financeWrite("/api/finance/wallets/", "POST", {
        organization: Number(form.organization), currency: "NGN", is_active: true,
        credit_limit: form.paymentModel === "credit" ? form.creditLimit : "0", notes: "Created through Finance Setup Wizard",
      });
      if (Number(form.openingBalance) > 0) {
        await financeWrite(`/api/finance/wallets/${wallet.id}/top-up/`, "POST", {
          amount: form.openingBalance, idempotency_key: `opening-${wallet.id}-${Date.now()}`,
          reference: "OPENING-BALANCE", description: "Opening balance entered during finance setup",
        });
      }
      setMessage(`Finance setup completed for ${selected?.name || "organisation"}.`); setStep(4);
