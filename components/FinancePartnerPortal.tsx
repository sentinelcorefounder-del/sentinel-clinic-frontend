...
export default function FinancePartnerPortal({ title }: { title: string }) {
  ...
  async function downloadDocument(item: BankTransferFunding, receipt = false) {
    setError("");
    try {
      const suffix = receipt ? "receipt-pdf" : "funding-request-pdf";
      const name = receipt ? item.receipt_reference : item.request_reference;
      await downloadFinancePdf(`/api/finance/bank-transfer-funding/${item.id}/${suffix}/`, `${name}.pdf`);
    } catch (err) {
  ...
  return (
    <main className="min-h-screen space-y-8 bg-slate-100 p-6 md:p-10">
        ...
        <div className="rounded-2xl border bg-white p-6 shadow-sm">
          ...
          {finance.wallet ? (
            <form onSubmit={topUp} className="mt-5 space-y-4">
              ...
              <label className="block text-sm font-medium">Amount (NGN)<input type="number" required min="1000" step="0.01" value={amount} onChange={(e) => setAmount(e.target.value)} className="mt-1 w-full rounded-xl border px-3 py-2" placeholder="50000" /></label>
      ...
      <section className="rounded-2xl border bg-white p-6 shadow-sm">
        ...
        {finance.wallet ? <form onSubmit={requestBankTransfer} className="mt-5 grid gap-3 md:grid-cols-3"><input required name="requested_amount" type="number" min="1000" step="0.01" placeholder="Requested amount (NGN)" className="rounded-xl border px-3 py-2"/><input name="notes" placeholder="Payment note (optional)" className="rounded-xl border px-3 py-2"/><button disabled={submitting} className="rounded-xl bg-slate-900 px-4 py-2 font-semibold text-white">Create transfer request</button></form> : null}
